  ...
  40  0020 0f            	dc.b	15
  41  0021 0f            	dc.b	15
  70                     ; 73 void AWU_DeInit(void)
  70                     ; 74 {
  72                     .text:	section	.text,new
  73  0000               _AWU_DeInit:
  77                     ; 75   AWU->CSR = AWU_CSR_RESET_VALUE;
  79  0000 725f50f0      	clr	20720
  80                     ; 76   AWU->APR = AWU_APR_RESET_VALUE;
  82  0004 353f50f1      	mov	20721,#63
  83                     ; 77   AWU->TBR = AWU_TBR_RESET_VALUE;
  85  0008 725f50f2      	clr	20722
  86                     ; 78 }
  89  000c 81            	ret	
 252                     ; 88 void AWU_Init(AWU_Timebase_TypeDef AWU_TimeBase)
 252                     ; 89 {
 253                     .text:	section	.text,new
 254  0000               _AWU_Init:
 256  0000 88            	push	a
 257       00000000      OFST:	set	0
 260                     ; 91   assert_param(IS_AWU_TIMEBASE_OK(AWU_TimeBase));
 262  0001 4d            	tnz	a
 ...
 304  0052               L41:
 305                     ; 94   AWU->CSR |= AWU_CSR_AWUEN;
 307  0052 721850f0      	bset	20720,#4
 308                     ; 97   AWU->TBR &= (uint8_t)(~AWU_TBR_AWUTB);
 310  0056 c650f2        	ld	a,20722
 311  0059 a4f0          	and	a,#240
 312  005b c750f2        	ld	20722,a
 313                     ; 98   AWU->TBR |= TBR_Array[(uint8_t)AWU_TimeBase];
 ...
 321                     ; 101   AWU->APR &= (uint8_t)(~AWU_APR_APR);
 323  006b c650f1        	ld	a,20721
 324  006e a4c0          	and	a,#192
 325  0070 c750f1        	ld	20721,a
 326                     ; 102   AWU->APR |= APR_Array[(uint8_t)AWU_TimeBase];
 ...
 337  0080 84            	pop	a
 338  0081 81            	ret	
 393                     ; 112 void AWU_Cmd(FunctionalState NewState)
 393                     ; 113 {
 394                     .text:	section	.text,new
 395  0000               _AWU_Cmd:
 399                     ; 114   if (NewState != DISABLE)
 401  0000 4d            	tnz	a
 402  0001 2705          	jreq	L531
 403                     ; 117     AWU->CSR |= AWU_CSR_AWUEN;
 405  0003 721850f0      	bset	20720,#4
 408  0007 81            	ret	
 409  0008               L531:
 410                     ; 122     AWU->CSR &= (uint8_t)(~AWU_CSR_AWUEN);
 ...
 471                     ; 139 void AWU_LSICalibrationConfig(uint32_t LSIFreqHz)
 471                     ; 140 {
 472                     .text:	section	.text,new
 473  0000               _AWU_LSICalibrationConfig:
 475  0000 5206          	subw	sp,#6
 476       00000006      OFST:	set	6
 479                     ; 141   uint16_t lsifreqkhz = 0x0;
 481                     ; 142   uint16_t A = 0x0;
 483                     ; 145   assert_param(IS_LSI_FREQUENCY_OK(LSIFreqHz));
 ...
 510  002e               L23:
 511                     ; 147   lsifreqkhz = (uint16_t)(LSIFreqHz / 1000); /* Converts value in kHz */
 ...
 522                     ; 151   A = (uint16_t)(lsifreqkhz >> 2U); /* Division by 4, keep integer part only */
 524  003f 54            	srlw	x
 525  0040 54            	srlw	x
 526  0041 1f05          	ldw	(OFST-1,sp),x
 527                     ; 153   if ((4U * A) >= ((lsifreqkhz - (4U * A)) * (1U + (2U * A))))
 529  0043 58            	sllw	x
 530  0044 58            	sllw	x
 ...
 546                     ; 155     AWU->APR = (uint8_t)(A - 2U);
 548  0064 a002          	sub	a,#2
 550  0066 2001          	jra	L361
 551  0068               L161:
 552                     ; 159     AWU->APR = (uint8_t)(A - 1U);
 554  0068 4a            	dec	a
 555  0069               L361:
 ...
 561  006e 81            	ret	
 584                     ; 168 void AWU_IdleModeEnable(void)
 584                     ; 169 {
 585                     .text:	section	.text,new
 586  0000               _AWU_IdleModeEnable:
 590                     ; 171   AWU->CSR &= (uint8_t)(~AWU_CSR_AWUEN);
 592  0000 721950f0      	bres	20720,#4
 593                     ; 174   AWU->TBR = (uint8_t)(~AWU_TBR_AWUTB);
 595  0004 35f050f2      	mov	20722,#240
 596                     ; 175 }
 599  0008 81            	ret	
 643                     ; 183 FlagStatus AWU_GetFlagStatus(void)
 643                     ; 184 {
 644                     .text:	section	.text,new
 645  0000               _AWU_GetFlagStatus:
 649                     ; 185   return((FlagStatus)(((uint8_t)(AWU->CSR & AWU_CSR_AWUF) == (uint8_t)0x00) ? RESET : SET));
