  ...
  33                     ; 67 void I2C_DeInit(void)
  33                     ; 68 {
  35                     .text:	section	.text,new
  36  0000               _I2C_DeInit:
  40                     ; 69   I2C->CR1 = I2C_CR1_RESET_VALUE;
  42  0000 725f5210      	clr	21008
  43                     ; 70   I2C->CR2 = I2C_CR2_RESET_VALUE;
  45  0004 725f5211      	clr	21009
  46                     ; 71   I2C->FREQR = I2C_FREQR_RESET_VALUE;
  48  0008 725f5212      	clr	21010
  49                     ; 72   I2C->OARL = I2C_OARL_RESET_VALUE;
  51  000c 725f5213      	clr	21011
  52                     ; 73   I2C->OARH = I2C_OARH_RESET_VALUE;
  54  0010 725f5214      	clr	21012
  55                     ; 74   I2C->ITR = I2C_ITR_RESET_VALUE;
  57  0014 725f521a      	clr	21018
  58                     ; 75   I2C->CCRL = I2C_CCRL_RESET_VALUE;
  60  0018 725f521b      	clr	21019
  61                     ; 76   I2C->CCRH = I2C_CCRH_RESET_VALUE;
  63  001c 725f521c      	clr	21020
  64                     ; 77   I2C->TRISER = I2C_TRISER_RESET_VALUE;
 ...
 245                     ; 96 void I2C_Init(uint32_t OutputClockFrequencyHz, uint16_t OwnAddress, 
 245                     ; 97               I2C_DutyCycle_TypeDef I2C_DutyCycle, I2C_Ack_TypeDef Ack, 
 245                     ; 98               I2C_AddMode_TypeDef AddMode, uint8_t InputClockFrequencyMHz )
 245                     ; 99 {
 246                     .text:	section	.text,new
 247  0000               _I2C_Init:
 249  0000 5209          	subw	sp,#9
 250       00000009      OFST:	set	9
 253                     ; 100   uint16_t result = 0x0004;
 255                     ; 101   uint16_t tmpval = 0;
 257                     ; 102   uint8_t tmpccrh = 0;
 259  0002 0f07          	clr	(OFST-2,sp)
 260                     ; 105   assert_param(IS_I2C_ACK_OK(Ack));
 ...
 294  0039 ae006c        	ldw	x,#108
 295  003c cd015d        	call	LC001
 296  003f               L24:
 297                     ; 109   assert_param(IS_I2C_INPUT_CLOCK_FREQ_OK(InputClockFrequencyMHz));
 299  003f 7b15          	ld	a,(OFST+12,sp)
 300  0041 2704          	jreq	L64
 301  0043 a111          	cp	a,#17
 302  0045 2506          	jrult	L05
 303  0047               L64:
 304  0047 ae006d        	ldw	x,#109
 305  004a cd015d        	call	LC001
 306  004d               L05:
 307                     ; 110   assert_param(IS_I2C_OUTPUT_CLOCK_FREQ_OK(OutputClockFrequencyHz));
 ...
 357                     ; 131     tmpccrh = I2C_CCRH_FS;
 359  009a a680          	ld	a,#128
 360  009c 6b07          	ld	(OFST-2,sp),a
 361                     ; 133     if (I2C_DutyCycle == I2C_DUTYCYCLE_2)
 363  009e 0d12          	tnz	(OFST+9,sp)
 364  00a0 96            	ldw	x,sp
 365  00a1 261d          	jrne	L121
 366                     ; 136       result = (uint16_t) ((InputClockFrequencyMHz * 1000000) / (OutputClockFrequencyHz * 3));
 ...
 386  00c0               L121:
 387                     ; 141       result = (uint16_t) ((InputClockFrequencyMHz * 1000000) / (OutputClockFrequencyHz * 25));
 ...
 431                     ; 157     I2C->TRISER = (uint8_t)tmpval;
 433  00f6 7b06          	ld	a,(OFST-3,sp)
 435  00f8 2028          	jra	L721
 436  00fa               L711:
 437                     ; 164     result = (uint16_t)((InputClockFrequencyMHz * 1000000) / (OutputClockFrequencyHz << (uint8_t)1));
 ...
 538  017d 81            	ret	
 593                     ; 202 void I2C_Cmd(FunctionalState NewState)
 593                     ; 203 {
 594                     .text:	section	.text,new
 595  0000               _I2C_Cmd:
 597  0000 88            	push	a
 598       00000000      OFST:	set	0
 601                     ; 205   assert_param(IS_FUNCTIONALSTATE_OK(NewState));
 603  0001 4d            	tnz	a
 604  0002 2711          	jreq	L201
 605  0004 4a            	dec	a
 606  0005 270e          	jreq	L201
 607  0007 ae00cd        	ldw	x,#205
 608  000a 89            	pushw	x
 609  000b 5f            	clrw	x
 610  000c 89            	pushw	x
...
1082  001a               L671:
1083                     ; 350   if (Ack == I2C_ACK_NONE)
1085  001a 7b01          	ld	a,(OFST+1,sp)
1086  001c 2606          	jrne	L533
1087                     ; 353     I2C->CR2 &= (uint8_t)(~I2C_CR2_ACK);
1089  001e 72155211      	bres	21009,#2
1091  0022 2011          	jra	L733
1092  0024               L533:
1093                     ; 358     I2C->CR2 |= I2C_CR2_ACK;
1095  0024 72145211      	bset	21009,#2
1096                     ; 360     if (Ack == I2C_ACK_CURR)
1098  0028 4a            	dec	a
1099  0029 2606          	jrne	L143
1100                     ; 363       I2C->CR2 &= (uint8_t)(~I2C_CR2_POS);
1102  002b 72175211      	bres	21009,#3
1104  002f 2004          	jra	L733
1105  0031               L143:
1106                     ; 368       I2C->CR2 |= I2C_CR2_POS;
1108  0031 72165211      	bset	21009,#3
1109  0035               L733:
1110                     ; 371 }
1113  0035 84            	pop	a
1114  0036 81            	ret	
1187                     ; 381 void I2C_ITConfig(I2C_IT_TypeDef I2C_IT, FunctionalState NewState)
1187                     ; 382 {
...
1306                     ; 408   assert_param(IS_I2C_DUTYCYCLE_OK(I2C_DutyCycle));
1308  0001 4d            	tnz	a
1309  0002 2712          	jreq	L232
1310  0004 a140          	cp	a,#64
1311  0006 270e          	jreq	L232
1312  0008 ae0198        	ldw	x,#408
...
1486  002f 81            	ret	
1517                     ; 458 void I2C_SendData(uint8_t Data)
1517                     ; 459 {
1518                     .text:	section	.text,new
1519  0000               _I2C_SendData:
1523                     ; 461   I2C->DR = Data;
1525  0000 c75216        	ld	21014,a
1526                     ; 462 }
1529  0003 81            	ret	
1750                     ; 578 ErrorStatus I2C_CheckEvent(I2C_Event_TypeDef I2C_Event)
1750                     ; 579 {
...
1759                     ; 580   __IO uint16_t lastevent = 0x00;
1761  0003 5f            	clrw	x
1762  0004 1f04          	ldw	(OFST-2,sp),x
1763                     ; 581   uint8_t flag1 = 0x00 ;
1765                     ; 582   uint8_t flag2 = 0x00;
1767                     ; 583   ErrorStatus status = ERROR;
1769                     ; 586   assert_param(IS_I2C_EVENT_OK(I2C_Event));
...
1813  0068               L072:
1814                     ; 588   if (I2C_Event == I2C_EVENT_SLAVE_ACK_FAILURE)
1816  0068 a30004        	cpw	x,#4
1817  006b 2609          	jrne	L706
1818                     ; 590     lastevent = I2C->SR2 & I2C_SR2_AF;
...
1826  0076               L706:
1827                     ; 594     flag1 = I2C->SR1;
1829  0076 c65217        	ld	a,21015
1830  0079 6b03          	ld	(OFST-3,sp),a
1831                     ; 595     flag2 = I2C->SR3;
1833  007b c65219        	ld	a,21017
1834  007e 6b06          	ld	(OFST+0,sp),a
1835                     ; 596     lastevent = ((uint16_t)((uint16_t)flag2 << (uint16_t)8) | (uint16_t)flag1);
...
1849                     ; 599   if (((uint16_t)lastevent & (uint16_t)I2C_Event) == (uint16_t)I2C_Event)
...
1967                     ; 648   return (I2C_Event_TypeDef)lastevent;
1969  0024 1e05          	ldw	x,(OFST-1,sp)
1972  0026 5b06          	addw	sp,#6
1973  0028 81            	ret	
2185                     ; 679 FlagStatus I2C_GetFlagStatus(I2C_Flag_TypeDef I2C_Flag)
2185                     ; 680 {
...
2194                     ; 681   uint8_t tempreg = 0;
2196  0002 4f            	clr	a
2197  0003 6b02          	ld	(OFST+0,sp),a
2198                     ; 682   uint8_t regindex = 0;
2200                     ; 683   FlagStatus bitstatus = RESET;
2202                     ; 686   assert_param(IS_I2C_FLAG_OK(I2C_Flag));
...
2245  0065               L403:
2246                     ; 689   regindex = (uint8_t)((uint16_t)I2C_Flag >> 8);
2248  0065 7b03          	ld	a,(OFST+1,sp)
2249  0067 6b01          	ld	(OFST-1,sp),a
2250                     ; 691   switch (regindex)
2253                     ; 708     default:
2253                     ; 709       break;
2254  0069 4a            	dec	a
...
2283                     ; 709       break;
2285  0083               L167:
2286                     ; 713   if ((tempreg & (uint8_t)I2C_Flag ) != 0)
2288  0083 7b04          	ld	a,(OFST+2,sp)
2289  0085 1502          	bcp	a,(OFST+0,sp)
2290  0087 2704          	jreq	L367
2291                     ; 716     bitstatus = SET;
2293  0089 a601          	ld	a,#1
2295  008b 2001          	jra	L567
2296  008d               L367:
2297                     ; 721     bitstatus = RESET;
2299  008d 4f            	clr	a
2300  008e               L567:
2301                     ; 724   return bitstatus;
2305  008e 5b04          	addw	sp,#4
2306  0090 81            	ret	
2349                     ; 759 void I2C_ClearFlag(I2C_Flag_TypeDef I2C_FLAG)
2349                     ; 760 {
...
2358                     ; 761   uint16_t flagpos = 0;
2360  0002 5f            	clrw	x
2361  0003 1f01          	ldw	(OFST-1,sp),x
2362                     ; 763   assert_param(IS_I2C_CLEAR_FLAG_OK(I2C_FLAG));
...
2378  001d               L413:
2379                     ; 766   flagpos = (uint16_t)I2C_FLAG & FLAG_Mask;
2381  001d 7b04          	ld	a,(OFST+2,sp)
2382  001f 5f            	clrw	x
2383  0020 02            	rlwa	x,a
2384  0021 1f01          	ldw	(OFST-1,sp),x
2385                     ; 768   I2C->SR2 = (uint8_t)((uint16_t)(~flagpos));
...
2394  002b 81            	ret	
2559                     ; 791 ITStatus I2C_GetITStatus(I2C_ITPendingBit_TypeDef I2C_ITPendingBit)
2559                     ; 792 {
...
2568                     ; 793   ITStatus bitstatus = RESET;
2570                     ; 794   __IO uint8_t enablestatus = 0;
2572  0003 0f03          	clr	(OFST-1,sp)
2573                     ; 795   uint16_t tempregister = 0;
2575                     ; 798     assert_param(IS_I2C_ITPENDINGBIT_OK(I2C_ITPendingBit));
...
2610  0051               L623:
2611                     ; 800   tempregister = (uint8_t)( ((uint16_t)((uint16_t)I2C_ITPendingBit & ITEN_Mask)) >> 8);
...
2624                     ; 803   enablestatus = (uint8_t)(I2C->ITR & ( uint8_t)tempregister);
2626  005f c6521a        	ld	a,21018
2627  0062 1402          	and	a,(OFST-2,sp)
2628  0064 6b03          	ld	(OFST-1,sp),a
2629                     ; 805   if ((uint16_t)((uint16_t)I2C_ITPendingBit & REGISTER_Mask) == REGISTER_SR1_Index)
...
2638                     ; 808     if (((I2C->SR1 & (uint8_t)I2C_ITPendingBit) != RESET) && enablestatus)
...
2646                     ; 811       bitstatus = SET;
2648  007d 200b          	jp	LC008
2649                     ; 816       bitstatus = RESET;
2650  007f               L3701:
2651                     ; 822     if (((I2C->SR2 & (uint8_t)I2C_ITPendingBit) != RESET) && enablestatus)
...
2659                     ; 825       bitstatus = SET;
2661  008a               LC008:
2663  008a a601          	ld	a,#1
2665  008c 2001          	jra	L1011
2666  008e               L3011:
2667                     ; 830       bitstatus = RESET;
2670  008e 4f            	clr	a
2671  008f               L1011:
2672                     ; 834   return  bitstatus;
2676  008f 5b06          	addw	sp,#6
2677  0091 81            	ret	
2721                     ; 871 void I2C_ClearITPendingBit(I2C_ITPendingBit_TypeDef I2C_ITPendingBit)
2721                     ; 872 {
...
2730                     ; 873   uint16_t flagpos = 0;
2732  0002 5f            	clrw	x
2733  0003 1f01          	ldw	(OFST-1,sp),x
2734                     ; 876   assert_param(IS_I2C_CLEAR_ITPENDINGBIT_OK(I2C_ITPendingBit));
...
2755  002e               L043:
2756                     ; 879   flagpos = (uint16_t)I2C_ITPendingBit & FLAG_Mask;
2758  002e 7b04          	ld	a,(OFST+2,sp)
2759  0030 5f            	clrw	x
2760  0031 02            	rlwa	x,a
2761  0032 1f01          	ldw	(OFST-1,sp),x
2762                     ; 882   I2C->SR2 = (uint8_t)((uint16_t)~flagpos);
